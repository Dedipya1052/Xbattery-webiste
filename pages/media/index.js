...
//   // Combine static featured blogs with contentful blogs (safely handling undefined or null)
//   const allBlogs = [...featuredStaticBlogs, ...(Array.isArray(blogs) ? blogs : [])];
  
//   // Sort blogs by date (most recent first)
//   const sortedBlogs = [...allBlogs].sort((a, b) => new Date(b.date) - new Date(a.date));
  
//   // Extract unique categories from all blogs
//   const categories = ["All", ...new Set(sortedBlogs.flatMap(blog => blog.categories || []))];
  
//   // Filter blogs based on search term and active category
//   useEffect(() => {
//     let results = sortedBlogs;
...
//   }, [searchTerm, activeCategory, sortedBlogs]);

//   // Set initial filtered blogs on component mount
//   useEffect(() => {
...
export default function Media({ blogs = [] }) {
  ...
  useEffect(() => {
    // Combine static featured blogs with contentful blogs
    const combinedBlogs = [...featuredStaticBlogs, ...(Array.isArray(blogs) ? blogs : [])];
