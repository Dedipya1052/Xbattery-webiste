...
// Custom Toast component - Updated for light theme
const Toast = ({ message, type }) => {
  return (
    <div className="fixed bottom-4 right-4 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div className={`px-4 py-3 rounded-lg shadow-lg ${
...
// Render options for Contentful rich text - Updated for light theme
const renderOptions = {
  renderNode: {
    [BLOCKS.PARAGRAPH]: (node, children) => {
      const textContent = node.content.map(item => item.value).join('');
      
      // Code block handling - Updated for light theme
      if (textContent.includes('[[code]]') && textContent.includes('[[*code]]')) {
        const codeText = textContent
          .replace('[[code]]', '')
          .replace('[[*code]]', '')
        ...
        return (
          <div className="relative group">
            <pre className="bg-gray-50 p-4 rounded-md my-4 overflow-x-auto border border-gray-200">
              <code className="text-sm font-mono text-blue-600">{codeText}</code>
              <button
                onClick={(e) => {
                  navigator.clipboard.writeText(codeText);
                  const tooltip = e.currentTarget.querySelector('.copy-tooltip');
                ...
                className="absolute top-3 right-3 p-2 rounded-md hover:bg-gray-200 text-gray-600 hover:text-gray-800 transition-all duration-200"
                aria-label="Copy code"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                ...
                <span
                  className="copy-tooltip opacity-0 absolute -top-8 right-0 bg-gray-700 text-white px-2 py-1 rounded text-xs whitespace-nowrap transform transition-all duration-300 ease-in-out"
                >
                  Copied!
                </span>
              </button>
...
export default function QuestionPage({ question, uniqueCategories }) {
  ...
  const getVoteButtonClass = (type) => {
    ...
    if (isThisVoteType) {
      return `${baseClass} ${type === 'upvote' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`;
    }
  ...
  return (
    <SupportLayout categories={allCategories}>
      ...
      <MathJaxContext>
        <div className="w-[100%] max-w-7xl mx-auto px-4 py-8 bg-white">
          {/* Breadcrumb */}
          <nav className="flex flex-wrap items-center gap-1 mb-8 text-sm text-gray-600 min-w-0">
            <div className="flex items-center min-w-0 flex-shrink-0">
              <button
                onClick={() => router.push("/support")}
            ...
            <span className="text-blue-600 truncate min-w-0 flex-1">
              {title}
            </span>
          </nav>
          ...
          <article className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 md:p-8">
            ...
            {/* Content */}
            <div className="prose max-w-none prose-pre:bg-gray-50 prose-pre:text-gray-800 prose-headings:text-gray-900 prose-p:text-gray-700">
              {documentToReactComponents(content, renderOptions)}
            </div>
