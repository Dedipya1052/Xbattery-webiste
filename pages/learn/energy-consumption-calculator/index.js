...
const ApplianceIcon = ({ appliance, onAdd, onRemove, onPowerChange, count }) => {
  ...
  return (
    <div className="flex flex-col items-center p-4 bg-white shadow-md rounded-lg transition-transform transform hover:scale-105 hover:shadow-lg">
      <Icon size={36} className="text-green-600 mb-3" />
      <span className="text-sm text-center text-gray-600 font-medium whitespace-nowrap overflow-hidden max-w-full">
        {appliance.name}
      </span>
      <div className="flex items-center mt-3">
        <input
          ...
          value={power}
          onChange={handlePowerChange}
          className="text-sm text-center text-gray-800 bg-gray-100 w-16 p-1 rounded-lg shadow-sm border border-gray-300"
        />
        <span className="ml-2 text-sm text-gray-500">W</span>
      </div>
      <div className="mt-4 flex items-center space-x-4">
...
const HomeEnergyCalculator = () => {
  ...
  useEffect(() => {
    ...
    Object.entries(appliances).forEach(([id, count]) => {
      const appliance = applianceTypes.find((a) => a.id === id);
      if (appliance) {
        totalConsumption += (appliance.power * 8 * 30 * count) / 1000;
      }
    });
    setConsumption(totalConsumption);
    setCost(totalConsumption * ELECTRICITY_RATE);
      ...
      <div className="min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Select Your Appliances:</h3>
          ...
          <div className="text-center mt-6 bg-gray-100 p-4 rounded-lg shadow-md">
          ...
          <p className="text-sm text-gray-500 text-center mt-4">
            Note: Adjust the wattage to match your devices.
          </p>
        </div>
      </div>
    </>
